...
class RelayClient(QObject):
    ...
    def __init__(self, database: LocalDatabase, download_dir: Path):
        ...
        self.connected_public_id: str | None = None
    ...
    @property
    def is_connected_as_current_identity(self) -> bool:
        return self.is_connected and self.identity is not None and self.connected_public_id == self.identity.public_id
    ...
    async def connect_to_server(
        ...
        *,
        ...
        connection_public_id = self.identity.public_id
    ...
    async def handle_server_frame(self, frame: dict[str, Any]) -> None:
        ...
        try:
            if self.connected_public_id is None:
                return
            if self.identity is not None and self.connected_public_id != self.identity.public_id:
                return
            if frame_type == "hello_ok":
                hello_public_id = frame.get("id")
                if isinstance(hello_public_id, str) and is_valid_x25519_public_id(hello_public_id):
                    self.connected_public_id = hello_public_id
                connection_public_id = self.connected_public_id or self.local_public_id
                await self.request_all_sync(
                    after_server_id=self.database.get_last_global_server_id(connection_public_id),
                    silent=True,
                )
            elif frame_type == "ack":
    ...
    async def _auto_sync_loop(self, connection_public_id: str) -> None:
        if self.stop_event is None:
            return
        while not self.stop_event.is_set():
            try:
                await asyncio.sleep(AUTO_SYNC_INTERVAL_SECONDS)
                if self.identity is None or self.identity.public_id != connection_public_id:
                    return
                if self.connected_public_id != connection_public_id:
                    return
                await self.request_all_sync(
                    after_server_id=self.database.get_last_global_server_id(connection_public_id),
                    silent=True,
    ...
    async def send_chat_message(self, peer_public_id: str, message_text: str) -> None:
        if self.identity is None:
            raise RuntimeError("identity is not loaded")
        if not self.is_connected_as_current_identity:
            raise RuntimeError("当前服务器连接不是当前身份，请重新连接后再发送。")
...
class MainWindow(QMainWindow):
    ...
    def load_identity_record(self, public_id: str) -> None:
        ...
        previous_public_id = self.relay.connected_public_id or (self.relay.identity.public_id if self.relay.identity else None)
        switching_connected_identity = self.relay.is_connected and previous_public_id != identity.public_id
        if switching_connected_identity:
            self.run_async(self.relay.disconnect(), "切换身份时断开旧连接失败")
            self._set_connection_button_state("disconnected")
            self.statusBar().showMessage("已切换身份；旧连接已断开，发送或同步时会使用新身份重新连接。")
    ...
    async def _ensure_connected(
        ...
        if self.relay.identity is None:
            raise RuntimeError("请先创建或选择身份。")
        if self._async_connection_lock is None:
            self._async_connection_lock = asyncio.Lock()
        async with self._async_connection_lock:
            if self.relay.is_connected and not self.relay.is_connected_as_current_identity:
    ...
    async def _connect_and_send_chat_message(
        self,
        host: str,
        port: int,
        proxy_config: Socks5ProxyConfig | None,
        ...
        await self._ensure_connected(host, port, proxy_config)
        await self.relay.send_chat_message(peer_public_id, message_text)

    async def _connect_and_send_file(
        self,
        host: str,
        port: int,
        proxy_config: Socks5ProxyConfig | None,
        ...
        await self._ensure_connected(host, port, proxy_config)
        await self.relay.send_file(peer_public_id, file_path)
    ...
    def send_message(self) -> None:
        ...
        self.run_async(
            self._connect_and_send_chat_message(host, port, proxy_config, self.current_peer_public_id, text),
            "发送失败",
        )
    ...
    def send_file_paths(self, files: list[str]) -> None:
        ...
        for file_name in files:
            ...
            if path.is_file():
                self.run_async(
                    self._connect_and_send_file(host, port, proxy_config, self.current_peer_public_id, path),
                    "文件发送失败",
                )
