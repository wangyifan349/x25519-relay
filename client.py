...
def prompt_yes_no(prompt: str, default: bool = False) -> bool:
    ...
    if not value:
        return default
    return value in {"y", "yes", "1", "true", "是", "对", "好"}
...
class FileReceiveManager:
    ...
    async def start_file(self, *, sender_id: str, payload: dict[str, Any]) -> None:
        ...
        print(f"\n正在接收文件: {filename} -> {destination_path}")
        ...
        async with receiving_file.lock:
            ...
            while receiving_file.next_expected_sequence in receiving_file.pending_chunks:
                ...
                if (
                    ...
                    print(f"文件接收中: {receiving_file.filename} {receiving_file.bytes_written}/{receiving_file.expected_size} bytes")
                    pass
    ...
    async def _finish_file_if_complete(self, receiving_file: ReceivingFile) -> None:
        ...
        if receiving_file.bytes_written != receiving_file.expected_size:
            print(f"文件大小校验失败: {receiving_file.filename}: got {receiving_file.bytes_written}, expected {receiving_file.expected_size}")
        elif actual_sha256 != receiving_file.expected_sha256:
            print(f"文件 sha256 校验失败: {receiving_file.filename}")
            print(f"       got:      {actual_sha256}")
            print(f"       expected: {receiving_file.expected_sha256}")
        else:
            print(f"\n文件已保存: {receiving_file.filename} -> {receiving_file.destination_path}")
...
class ChatClient:
    ...
    async def run(self) -> None:
        print("当前对话对象已设置。")
        print("发送显示 >>>，接收显示 <<<。会自动同步历史；/send、/file、/files 发送文件。")
    ...
    def print_help(self) -> None:
        print("用法：")
        print("  直接输入文字                 发送聊天消息，显示为 >>> 内容")
        print("  /send <路径1> [路径2 ...]    发送一个或多个文件")
        print("  /file <路径1> [路径2 ...]    发送一个或多个文件")
        print("  /files <路径1> [路径2 ...]   发送一个或多个文件")
        print("  /to <对方公钥/Base58>       切换对话对象")
        print("  /id                         显示自己的 Base58 公钥")
        print("  /fingerprint                显示自己的公钥指纹")
        print("  /peerfp                     显示当前对方指纹，用于必要时核对")
        print("  /sync                       同步当前对话，恢复历史消息和文件")
        print("  /quit                       退出")
        print("路径示例：")
            ...
            with file_path.open("rb") as file_handle:
                while True:
                    ...
                    if sequence_number % 64 == 0 or queued_bytes == file_size:
                        print(f"文件发送排队中: {file_path.name} {queued_bytes}/{file_size} bytes")
                        pass

            print(f"文件已加入发送队列: {file_path.name}")
            pass

        except asyncio.CancelledError:
            raise
        except Exception as exc:
            print(f"文件发送失败: {file_path}: {exc}")
            pass
        finally:
...
def parse_socks5_proxy_config(raw_proxy_value: str) -> Socks5ProxyConfig | None:
    ...
    if username is None:
        username_input = input("SOCKS5 用户名，可留空: ").strip()
        password_input = input("SOCKS5 密码，可留空: ").strip() if username_input else ""
...
def prompt_peer_public_id() -> str:
    ...
    """
    while True:
        peer_public_id = normalize_public_id(input("你现在想发给谁，请输入对方公钥/Base58: ").strip())
        if is_valid_x25519_public_id(peer_public_id):
            return peer_public_id
        print("公钥格式不正确。请输入 32 字节 X25519 公钥的 Base58 或旧版 base64url 字符串。")
...
def prompt_startup_config() -> tuple[Path, str, int, Socks5ProxyConfig | None, str]:
    ...
    server_host = prompt_line("服务器地址", "127.0.0.1")
    while server_host.startswith("tcp://"):
        server_host = server_host.removeprefix("tcp://")
    server_port_text = prompt_line("服务器端口", "8765")
    try:
        server_port = int(server_port_text)
    except ValueError:
        raise ValueError("服务器端口必须是整数")

    peer_public_id = prompt_peer_public_id()

    raw_proxy_value = input("SOCKS5 TCP 代理，可选，留空不用，例如 127.0.0.1:1080: ").strip()
    proxy_config = parse_socks5_proxy_config(raw_proxy_value)

    identity_path = Path("identity.json")
    if proxy_config:
        print(f"使用 SOCKS5 代理: {proxy_config.host}:{proxy_config.port}")
    else:
        pass
