    ...
    def _initialize_schema(self) -> None:
        with self.lock:
            ...
            self.connection.execute(
                """
                -- Persistent encrypted relay store.
                --
                -- The server never decrypts envelope_json. It stores encrypted
                -- client envelopes plus the minimum routing/sync metadata needed
                -- to relay messages and reconstruct conversations.
                CREATE TABLE IF NOT EXISTS messages (
                    -- Monotonic server-side id used for pagination and sync.
                    server_id INTEGER PRIMARY KEY AUTOINCREMENT,

                    -- Server receive timestamp in milliseconds.
                    created_at_ms INTEGER NOT NULL,

                    -- Sender and recipient public ids, used only for routing.
                    sender_id TEXT NOT NULL,
                    recipient_id TEXT NOT NULL,

                    -- Encrypted envelope type: chat, file_start, or file_chunk.
                    kind TEXT NOT NULL,

                    -- Client-generated id used for idempotent insert/retry.
                    client_message_id TEXT NOT NULL,

                    -- End-to-end encrypted envelope JSON. The server cannot
                    -- decrypt chat text, filenames, file hashes, or file bytes.
                    envelope_json TEXT NOT NULL,

                    -- Size/hash are for accounting, dedup diagnostics, and
                    -- defensive validation; they are computed over ciphertext.
                    envelope_size INTEGER NOT NULL,
                    envelope_sha256 TEXT NOT NULL,

                    -- For self_copy rows, this is the peer whose conversation
                    -- the self-copy belongs to. It lets the sender sync back
                    -- their own outgoing history for one peer.
                    conversation_peer_id TEXT,

                    -- delivery  = encrypted for the peer
                    -- self_copy = encrypted for the sender's own future sync
        ...
        with self.lock:
            try:
                cursor = self.connection.execute(
                    """
                    -- Store exactly one encrypted envelope.
                    --
                    -- copy_role distinguishes peer delivery copies from
                    -- sender-owned self copies. Both are ciphertext; the server
                    -- treats them the same for persistence but differently for
                    -- sync selection.
                ...
                existing = self.connection.execute(
                    """
                    -- If the client retries the same message after a network
                    -- failure, return the original row instead of inserting a
                    -- duplicate envelope.
        ...
        *,
        ...
        with self.lock:
            rows = self.connection.execute(
                """
                -- Conversation sync returns only envelopes that this user can
                -- decrypt:
                --
                -- 1. delivery rows sent by the peer to this user;
                -- 2. self_copy rows created by this user for this conversation.
                --
                -- It intentionally does not return this user's old delivery
                -- rows encrypted to the peer, because this user cannot decrypt
                -- those peer-targeted ciphertexts after reconnecting.
                SELECT server_id, created_at_ms, sender_id, recipient_id, kind, envelope_json,
                       conversation_peer_id, copy_role
                FROM messages
                WHERE server_id > ?
    ...
    def fetch_all_for_user(
        ...
        *,
        ...
        with self.lock:
            rows = self.connection.execute(
                """
                -- Global sync returns all decryptable envelopes addressed to
                -- this identity: peer delivery copies and sender self copies.
                SELECT server_id, created_at_ms, sender_id, recipient_id, kind, envelope_json,
                       conversation_peer_id, copy_role
                FROM messages
                WHERE server_id > ?
...
class RelayServer:
    ...
    async def handle_send_frame(self, session: ClientSession, frame: dict[str, Any]) -> None:
        try:
            ...
            # Persist first, then attempt live delivery. If live delivery fails,
            # the recipient can still recover the encrypted envelope by sync.
    ...
    async def handle_send_self_copy_frame(self, session: ClientSession, frame: dict[str, Any]) -> None:
        ...
        try:
            ...
            # Self copies are never delivered to the peer. They are encrypted
            # to the sender and returned only during the sender's future sync.
            stored_message = await asyncio.to_thread(
                self.message_store.insert_envelope,
                sender_id=session.public_id,
                recipient_id=session.public_id,
...
def prompt_server_config() -> tuple[str, int, ServerConfig]:
    ...
    listen_host = prompt_line("Server listen address", "0.0.0.0")
    listen_port = prompt_int("Server listen port", 8765, minimum=1)
    database_path = prompt_line("SQLite database file", "messages.sqlite3")

    print()
    print("Advanced settings follow. Press Enter to use recommended defaults.")
    max_tcp_frame_size = prompt_int(
        "Maximum bytes per TCP frame",
        DEFAULT_MAX_TCP_FRAME_SIZE,
        minimum=1024,
    )
    max_envelope_json_size = prompt_int(
        "Maximum JSON bytes per encrypted envelope",
        DEFAULT_MAX_ENVELOPE_JSON_SIZE,
        minimum=1024,
    )
    urgent_queue_limit = prompt_int(
        "Urgent queue limit per connection",
        DEFAULT_URGENT_QUEUE_LIMIT,
        minimum=1,
    )
    bulk_queue_limit = prompt_int(
        "Bulk queue limit per connection",
        DEFAULT_BULK_QUEUE_LIMIT,
        minimum=1,
    )
    max_sync_limit = prompt_int(
        "Maximum messages per sync page",
